...
class LiveDataBuffer:
    """
    Class designed to efficiently store and manage the most recent 
    streamed data in memory. This provides methods to update the 
    buffer with new data as it arrives, as well as access methods 
    to retrieve the stored data for analysis or further processing.

    Args:
        crawler: Iterable which will crawl through a directory
            containing a stream of frame data and will
            return the frame filenames in chronological order
            based on the timestamps in their filenames.
    ...
    def __init__(self, crawler: Iterable, channels: Union[str, Iterable[str]]):
        ...
        self.analysis_duration = self.target_duration + 2 * self.edge_duration
        
        self.data = load_frame(fname=str(next(self.crawler)),
                               channels=self.channels)
        
        while self.data.shape[1]/self.sample_rate < self.analysis_duration:            
            new_data = load_frame(
                                  fname=str(next(self.crawler)),
                                  channels=self.channels)
